...
const Dashboard: NextPageWithLayout<IDashboard> = () => {
  ...
  const handleNewBucket = async () => {
    if (user) {
      firestore.createBucket(user.id, newBucketId)
        .then((bucket) => {
  ...
  const handleRemoveBucket = async (bucketId: string) => {
    if (user) {
      firestore.removeBucket(bucketId)
        .then(() => {
                ...
                <div className="flex mt-4">
                  ...
                  >
                    ...
                    <ChangeModal
                      ...
                      changeInputPlaceholder="New Bucket"
                      error={error}
                      onClickCancel={() => {
                        onClose();
                        setError('');
